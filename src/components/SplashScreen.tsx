...
export default function SplashScreen({ onComplete }: SplashScreenProps) {
  ...
  // Terminal Typing Log Emulator Sequence (exactly matching OLED timing specs)
  useEffect(() => {
    const bootLogs = [
  ...
  // Full-Schermata Canvas Particle Engine and Phase Timeline
  useEffect(() => {
    ...
    const generateTextTargetsAndParticles = () => {
      ...
      // Defensive constraints guard against 0 dimension in nested frames
      if (width <= 0 || height <= 0) return;

      try {
        // Calculate crisp text sizes matching precise screen math
    ...
    safeTimeout(() => {
      animationStage = 2; // stage = 2 starts collapsing the scattered particles
    }, 2000);

    safeTimeout(() => {
      globalTransitionAlpha = 0.01;
      safeTimeout(() => {
        animationStage = 3; // Solidy white text overlay
      }, 100);

      setSloganVisible(true);
      setLoadingBarVisible(true);

      // Bloomberg style progression loading bar (4% progress every 45ms)
      let progress = 0;
      const progressInterval = safeInterval(() => {
        ...
        if (progress >= 100) {
          ...
          // Once loading bar hits 100%, trigger gorgeous biometric cyber glow
          safeTimeout(() => {
            setInternalTransitionTrigger(true);
            safeTimeout(() => {
              onCompleteRef.current();
            }, 400); // Fluid out scale/blur transit
          }, 300);
        }
      }, 45);
    }, 4500);
  ...
  return (
    ...
    >
      ...
      {/* Slogan Container matching absolute oled text ratios */}
      <div 
